...
describe("CompanyService", () => {
  ...
  describe("createCompany", () => {
    it("should create a new company if it does not exist", async () => {
      ...
      CompanyModel.createCompany.mockResolvedValue({
        id: 1,
        name: "Nodalis",
        address: "Quito",
      });
    ...
    it("should throw if company already exists", async () => {
      CompanyModel.getCompanyByName.mockResolvedValue({
        id: 2,
        name: "Nodalis",
      });

      await expect(
        CompanyService.createCompany("Nodalis", "Quito"),
      ).rejects.toThrow("La empresa con el nombre Nodalis ya existe");
    });
  });

  describe("updateCompany", () => {
    it("should update a company by id", async () => {
      CompanyModel.updateCompany.mockResolvedValue({
        id: 1,
        name: "Kaeli Corp",
        address: "Bruxelles",
      });
      ...
      expect(CompanyModel.updateCompany).toHaveBeenCalledWith(
        1,
        "Kaeli Corp",
        "Bruxelles",
      );
  ...
  describe("deleteCompany", () => {
    ...
    it("should throw generic delete error otherwise", async () => {
      CompanyModel.deleteCompany.mockRejectedValue(
